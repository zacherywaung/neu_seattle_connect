...
export default function CourseDetail() {
  ...
  const workloadAvg = threads.length > 0
    ? threads.reduce((sum, t) => sum + (workloadMap[t.workload] || 2), 0) / threads.length
    : null;
  const dominantWorkload = workloadAvg ? reverseMap(workloadAvg) : null;
    ...
    <div className="min-h-screen bg-[#f8f8f8]">
      ...
      <div className="bg-[#C8102E] text-white py-12">
        <div className="max-w-4xl mx-auto px-4">
          <button onClick={() => navigate('/courses')} className="text-white/70 hover:text-white text-sm mb-4 inline-flex items-center gap-1">
            ← Back to all courses
          </button>
          <h1 className="text-3xl font-bold">{code.toUpperCase()}</h1>
          <p className="text-white/80 mt-1">{threads.length} {threads.length === 1 ? 'insight' : 'insights'} shared</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {!loading && threads.length > 0 && (
          <div className="bg-white border border-[#e5e5e5] rounded-xl p-6 mb-6">
            <h2 className="text-sm font-semibold text-[#111111] mb-4 uppercase tracking-wide">At a glance</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">

              {/* Workload — weighted average + distribution */}
              <div>
                <p className="text-xs text-[#999] mb-2">Workload</p>
                {dominantWorkload && (
                  <>
                    <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${workloadStyle[dominantWorkload]}`}>
                      {dominantWorkload}
                    </span>
                    <p className="text-xs text-[#999] mt-1 mb-3">based on {total} {total === 1 ? 'review' : 'reviews'}</p>

                    {/* Distribution bars */}
                    <div className="space-y-1.5">
                      {['Light', 'Medium', 'Heavy'].map(w => {
                        const count = workloadCount[w] || 0;
                        const pct   = total > 0 ? Math.round((count / total) * 100) : 0;
                        const barColor = w === 'Light' ? 'bg-green-400' : w === 'Medium' ? 'bg-yellow-400' : 'bg-red-400';
                        return (
                          <div key={w} className="flex items-center gap-2">
                            <span className="text-xs text-[#999] w-12">{w}</span>
                            <div className="flex-1 h-1.5 bg-[#f0f0f0] rounded-full overflow-hidden">
                              <div className={`h-full rounded-full ${barColor} transition-all`} style={{ width: `${pct}%` }} />
                            </div>
                            <span className="text-xs text-[#999] w-6 text-right">{count}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div>
                ...
                {aiSummary?.learningStyle
                  ...
                  : <p className="text-xs text-[#999]">Available after 3+ reviews</p>
                }
              </div>
        ...
        {/* Share button */}
        {token && !showForm && (
          <button onClick={() => setShowForm(true)} className="mb-6 px-6 py-3 bg-[#C8102E] text-white font-semibold rounded-lg hover:bg-[#a00d24] transition-colors">
            + Share My Experience
          </button>
        )}
        ...
        {!token && (
          <div className="mb-6 bg-white border border-[#e5e5e5] rounded-lg p-4 flex items-center justify-between">
            ...
            <button onClick={() => navigate('/login')} className="px-4 py-2 bg-[#C8102E] text-white text-sm font-semibold rounded-lg hover:bg-[#a00d24] transition-colors">
              Log in
            </button>
          </div>
        )}
        ...
        {showForm && (
          <div className="mb-6 bg-white border border-[#e5e5e5] rounded-lg p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#111111] mb-1">Course name <span className="text-[#999] font-normal">(optional)</span></label>
                <input type="text" name="courseName" value={form.courseName} onChange={handleChange} placeholder="e.g. Algorithms"
              ...
              <div>
                <label className="block text-sm font-medium text-[#111111] mb-1">Learning style <span className="text-[#999] font-normal">(optional)</span></label>
                <input type="text" name="learningStyle" value={form.learningStyle} onChange={handleChange} placeholder="e.g. Lecture-heavy, project-based, self-study..."
                  className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-[#111111] placeholder-[#999] focus:outline-none focus:border-[#C8102E] focus:ring-1 focus:ring-[#C8102E]" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-[#111111] mb-1">Career relevance <span className="text-[#999] font-normal">(optional)</span></label>
                <input type="text" name="careerRelevance" value={form.careerRelevance} onChange={handleChange} placeholder="e.g. Very useful for SWE interviews"
                  className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-[#111111] placeholder-[#999] focus:outline-none focus:border-[#C8102E] focus:ring-1 focus:ring-[#C8102E]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#111111] mb-1">Key takeaway <span className="text-[#C8102E]">*</span></label>
            ...
            {threads.map((t) => (
              <div key={t._id} className="bg-white border border-[#e5e5e5] rounded-lg p-6 hover:shadow-sm transition-shadow">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#555555]">{[t.author?.major, t.author?.year].filter(Boolean).join(' · ')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {currentUserId && t.author?._id === currentUserId && (
                      <button onClick={() => handleDelete(t._id)} className="text-xs text-[#999] hover:text-red-600 transition-colors px-2 py-1">✕</button>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {t.learningStyle && (
                    <span className="text-xs bg-[#f8f8f8] text-[#555555] px-3 py-1 rounded-full border border-[#e5e5e5]">📖 {t.learningStyle}</span>
                  )}
                  {t.careerRelevance && (
                    <span className="text-xs bg-[#f8f8f8] text-[#555555] px-3 py-1 rounded-full border border-[#e5e5e5]">💼 {t.careerRelevance}</span>
                  )}
                </div>
